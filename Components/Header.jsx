...
const Header = () => {
  ...
  return (
    <header className="bg-white shadow fixed top-0 z-50 w-full">
      {/* Top info bar */}
      <div className="bg-gray-800 text-white p-2 flex flex-col md:flex-row items-center md:justify-between text-sm">
        ...
        <div className="flex gap-4 px-4 mt-2 md:mt-0 text-xl">
          ...
          <a href="https://www.youtube.com/@technicalinstituteofpatna" target="_blank" rel="noopener noreferrer">
            <FaYoutube className="text-red-600 rounded-full hover:scale-110 transition" />
          </a>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center px-4 md:px-10 bg-white relative">
        ...
        <ul className="hidden md:flex items-center gap-6 text-lg">
          <li className="hover:text-gray-800 font-bold"><Link href="/">Home</Link></li>
          <li className="hover:text-gray-800 font-bold"><Link href="/service">Courses</Link></li>
          <li className="hover:text-gray-800 font-bold"><Link href="/about">About Us</Link></li>
          <li className="hover:text-gray-800 font-bold"><Link href="/contact">Contact Us</Link></li>
        </ul>
      ...
      <div className={`fixed top-0 right-0 h-full w-2/3 max-w-xs bg-white shadow-md p-6 transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <ul className="flex flex-col gap-6 text-lg mt-16">
          <li className='text-gray-800'><Link href="/" onClick={() => setIsOpen(false)}>Home</Link></li>
          <li className='text-gray-800'><Link href="/service" onClick={() => setIsOpen(false)}>Courses</Link></li>
          <li className='text-gray-800'><Link href="/about" onClick={() => setIsOpen(false)}>About Us</Link></li>
          <li className='text-gray-800'><Link href="/contact" onClick={() => setIsOpen(false)}>Contact Us</Link></li>
        </ul>
      </div>
